...
function getUploadStatusFlags(uploadStatus: SupportUploadStatus) {
  return {
    isConfirmed: uploadStatus === "confirmed" || uploadStatus === "linked",
    isError: uploadStatus === "error",
    isPending: uploadStatus === "pending" || uploadStatus === "idle",
    statusLabel: getStatusLabel(uploadStatus),
  };
}

type TSupportAttachmentPreviewVariantProps = {
  attachment: SupportImageAttachment;
  disabled: boolean;
  formatFileSize: (bytes: number) => string;
  isConfirmed: boolean;
...
const SupportAttachmentPreviewCompact = memo(
  ({
    attachment,
    disabled,
    isConfirmed,
    ...
    onRetry,
    removeDisabled,
  }: Omit<TSupportAttachmentPreviewVariantProps, "formatFileSize" | "statusLabel">) => (
...
SupportAttachmentPreviewCompact.displayName = "SupportAttachmentPreviewCompact";

const SupportAttachmentPreviewDefault = memo(
  ({
    attachment,
    disabled,
    formatFileSize,
    ...
    removeDisabled,
    statusLabel,
  }: TSupportAttachmentPreviewVariantProps) => (
...
SupportAttachmentPreviewDefault.displayName = "SupportAttachmentPreviewDefault";
...
export const SupportAttachmentPreview = memo(
  ({
    ...
    const { isConfirmed, isError, isPending, statusLabel } = getUploadStatusFlags(
    ...
    if (variant === "compact") {
      return <SupportAttachmentPreviewCompact {...sharedProps} />;
    }

    return (
      <SupportAttachmentPreviewDefault
        {...sharedProps}

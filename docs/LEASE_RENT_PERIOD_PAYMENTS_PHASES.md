...
- [x] Stripe `applyIncomeForFullyCoveredMonths`: set period key on created lines; books **`allocated_cents`** per payment (partial Record Rent + Stripe remainder both appear on Income; idempotent per payment + period)
